...
week1_section = '''## Week 1 Delivery Timing & Rolling Free Cadence
...
if count != 1:
    raise SystemExit(f'Week 1 timing section replacement count={count}')

old_anchor = "For the first packet only, successful Finisher completion may advance the actual Week 1 `release_at`. The Week 2 release and generation deadlines must then be derived from that actual Week 1 release anchor plus seven days, rather than from the superseded next-day expectation."
new_anchor = "For the first packet only, successful Week 1 Fast Publisher completion sets the actual Week 1 `release_at` to publication time. The Week 2 release and generation deadlines are then derived from that actual Week 1 release anchor plus seven days. The normal Finisher is not a Week 1 publication path."
if old_anchor not in text:
    raise SystemExit('Week 1 release-anchor sentence not found')
text = text.replace(old_anchor, new_anchor, 1)

first_material = '''# 193. Definition of Done: First Material
...
'''
text, count = re.subn(
    r'# 193\. Definition of Done: First Material\n.*?\n---\n\n# 194\.',
    first_material + '\n---\n\n# 194.',
    text,
    count=1,
    flags=re.S,
...
if count != 1:
    raise SystemExit(f'Section 193 replacement count={count}')

generation_reliability = '''# 199. Definition of Done: Generation Reliability
...
'''
text, count = re.subn(
    r'# 199\. Definition of Done: Generation Reliability\n.*?\n---\n\n# 200\.',
...
if count != 1:
    raise SystemExit(f'Section 199 replacement count={count}')

marker = 'For weekly-material work specifically:\n'
exception = '''For weekly-material work specifically:
...
if marker not in text:
    raise SystemExit('Section 205 introduction not found')
text = text.replace(marker, exception, 1)
text = text.replace(
    'preserve valid unrelated content, immutable attempts, retry semantics, and Claim/Submit/Finisher boundaries.',
    'preserve valid unrelated content, immutable attempts, retry semantics, and the applicable Claim/Submit/publication boundary (Week 1 Fast Publisher for the first packet; normal Finisher for Week 2+).',
    1,
)

architecture = '''# 206. Core Architectural Summary
...
'''
text, count = re.subn(
    r'# 206\. Core Architectural Summary\n.*?\n---\n\n# 207\.',
    architecture + '\n---\n\n# 207.',
    text,
    count=1,
    flags=re.S,
...
if count != 1:
    raise SystemExit(f'Section 206 replacement count={count}')

forbidden = [
    '第一份專屬教材預計隔天開放下載',
    '預計 8月17日 交付第一份教材',
    'For the first packet only, successful Finisher completion',
    'sole curriculum author (the repository-owned local Windows Codex CLI runner)',
]
leftovers = [item for item in forbidden if item in text]
if leftovers:
    raise SystemExit(f'stale Week 1 contract remains: {leftovers}')

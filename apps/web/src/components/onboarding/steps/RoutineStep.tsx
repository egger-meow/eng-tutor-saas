...
export function RoutineStep({ draft, errors, update }: OnboardingStepProps) {
  return <>
    <label>每週可安排多少分鐘<input type="number" min={20} max={1200} value={draft.weeklyMinutes} onChange={(event) => update({ weeklyMinutes: Number(event.target.value) })} aria-invalid={Boolean(errors.weeklyMinutes)} />{errors.weeklyMinutes && <span className="field-error">{errors.weeklyMinutes}</span>}</label>

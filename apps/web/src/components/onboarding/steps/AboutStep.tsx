...
export function AboutStep({ draft, errors, update }: OnboardingStepProps) {
  ...
  return <>
    <label>孩子暱稱 <small className="field-hint">（小名或英文名）</small><input autoFocus maxLength={80} placeholder="例如：Jonathan, 翔翔" value={draft.displayName} onChange={(event) => update({ displayName: event.target.value })} aria-invalid={Boolean(errors.displayName)} />{errors.displayName && <span className="field-error">{errors.displayName}</span>}</label>

...
export function ScopedMaterialLoadingState() {
  return <div className="loading-state scoped-material-loading-state" role="status"><div className="loading-spinner" /><p>正在安全開啟教材…</p></div>
}
...
export function ScopedMaterialPage({ session }: { session: Session | null }) {
  ...
  return <><PublicHeader /><main className={`section scoped-material-main${state.status === 'loading' ? ' scoped-material-main-loading' : ''}`}><div className="container scoped-material-page">
    {state.status === 'loading' && <ScopedMaterialLoadingState />}

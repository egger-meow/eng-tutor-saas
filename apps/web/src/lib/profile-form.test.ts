...
describe('profile form', () => {
  it('requires only generation-critical fields', () => {
    ...
    expect(validateProfileStep(1, emptyProfileDraft)).toHaveProperty('baselineLevel')
    expect(validateProfileStep(2, emptyProfileDraft)).toEqual({})
    expect(validateProfileStep(3, emptyProfileDraft)).toEqual({})
  })

  it('accepts only a single-packet weekly workload domain', () => {
    expect(validateProfileStep(3, { ...emptyProfileDraft, weeklyMinutes: 30 })).toEqual({})
    expect(validateProfileStep(3, { ...emptyProfileDraft, weeklyMinutes: 240 })).toEqual({})
    expect(validateProfileStep(3, { ...emptyProfileDraft, weeklyMinutes: 29 })).toHaveProperty('weeklyMinutes')
    expect(validateProfileStep(3, { ...emptyProfileDraft, weeklyMinutes: 241 })).toHaveProperty('weeklyMinutes')
  ...
  it('maps flexible context into preferences without losing core profile fields', () => {
    ...
    expect(input.learning_goals).toBe('read independently')
    expect(input.preferences).toMatchObject({ interests: ['動物'], upcomingTest: '9/15' })
  })

  it('provides default learning goal when left empty', () => {
    const input = toChildProfileInput({ ...emptyProfileDraft, baselineLevel: 'grade-7', learningGoals: '' })
    expect(input.learning_goals).toContain('建立自主閱讀習慣')
  })

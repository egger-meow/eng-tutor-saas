...
import {
  ...
  computeFrozen2130Hashes,
  computeFrozen2131Hashes,
  computeFrozen2132Hashes,
  computeFrozen2140Hashes,
  REPO_ROOT,
} from './bundle-compiler.js'

describe('bundle-compiler', () => {
  it('generates a compact deterministic 2.14.1 production bundle with no historical overlay sediment', async () => {
  ...
  expect(freshBundle.metadata.schemaVersion).toBe('2.6.0')
  expect(freshBundle.metadata.promptVersion).toBe('2.14.1')
  expect(freshBundle.metadata.bundleVersion).toBe('2.14.1-prod')
  expect(freshBundle.metadata.engineVersion).toBe('1.9.0')
  expect(Object.keys(freshBundle.metadata.sourceHashes).length).toBe(15)
  expect(freshBundle.metadata.sourceHashes).toHaveProperty('packages/generator/prompts/2.14.1/01-plan.md')
  expect(freshBundle.metadata.sourceHashes).toHaveProperty('packages/generator/prompts/2.14.1/03-critic.md')
...
it('verifies that prompts/2.11.0 consolidated baseline remains byte-for-byte frozen after the patch bump', async () => {
  expect(await computeFrozen2110Hashes(REPO_ROOT)).toEqual({
...
it('verifies that prompts/2.13.0 consolidated baseline remains byte-for-byte frozen after the 2.13.1 bump', async () => {
  expect(await computeFrozen2130Hashes(REPO_ROOT)).toEqual({
...
it('verifies that prompts/2.13.1 consolidated baseline remains byte-for-byte frozen after the 2.13.2 bump', async () => {
  expect(await computeFrozen2131Hashes(REPO_ROOT)).toEqual({
...
it('verifies that prompts/2.13.2 consolidated baseline remains byte-for-byte frozen', async () => {
  expect(await computeFrozen2132Hashes(REPO_ROOT)).toEqual({
    'packages/generator/prompts/2.13.2/01-plan.md': '728395e2890e051cc2a1223dac1509520ad0c4b4e06c9a02318520002686ad05',
    'packages/generator/prompts/2.13.2/02-author.md': '627960504ff8f04cf9ea39b13665f9785acb66b37aac8648cebcb25fe4e03e57',
    'packages/generator/prompts/2.13.2/03-critic.md': '6674390f129c23b187d2a4706e61ec8bab7c8596339a65cb070c0234d5691a92',
    'packages/generator/prompts/2.13.2/04-repair.md': 'e4b826f640e93590dab0c7a61739e040dda74c25b49bf2f9bdfe435fbca2327e',
  })
})

it('verifies that activated prompts/2.14.0 remain byte-for-byte frozen after the 2.14.1 release', async () => {
  expect(await computeFrozen2140Hashes(REPO_ROOT)).toEqual({

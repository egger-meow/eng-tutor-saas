...
describe('bundle-compiler', () => {
  it('generates a deterministic production bundle with matching source hashes and no drift', async () => {
    ...
    expect(freshBundle.metadata.promptVersion).toBe('2.9.0')
    expect(freshBundle.metadata.bundleVersion).toBe('2.9.0-prod')
    ...
    expect(freshBundle.content).toContain('A weaker learner may receive A1/A2 language with D2/D3 thinking')

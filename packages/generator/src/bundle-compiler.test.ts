...
import { compileProductionBundle, computeFrozen201Hashes, computeFrozen210Hashes, REPO_ROOT } from './bundle-compiler.js'
...
describe('bundle-compiler', () => {
  it('generates a deterministic production bundle with matching source hashes and no drift', async () => {
    ...
    expect(freshBundle.metadata.promptVersion).toBe('2.2.0')
    expect(freshBundle.metadata.bundleVersion).toBe('2.2.0-prod')

...
describe('Prompt 2.14.1 consolidated active invariant contract with frozen historical provenance', () => {

  it('keeps the active production prompt compact while covering the generalized exact-attribution hole', async () => {
    const plan = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.14.1/01-plan.md'), 'utf8')
    const author = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.14.1/02-author.md'), 'utf8')
    const critic = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.14.1/03-critic.md'), 'utf8')
    const repair = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.14.1/04-repair.md'), 'utf8')

    for (const stage of [plan, author, critic, repair]) {
      expect(stage).toContain('2.14.1')
  ...
  it('uses CurriculumPackageSchema 2.6.0 while preserving V25, V24, V23, V22, V21, and V20 legacy schemas', async () => {
    ...
    expect(bundle).toContain('bundleVersion: "2.14.1-prod"')

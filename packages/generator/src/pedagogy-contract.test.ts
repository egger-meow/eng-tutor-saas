...
describe('Prompt 2.12.0 consolidated active invariant contract with frozen historical provenance', () => {

  it('keeps the active production prompt compact while covering the generalized exact-attribution hole', async () => {
    const plan = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.12.0/01-plan.md'), 'utf8')
    const author = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.12.0/02-author.md'), 'utf8')
    const critic = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.12.0/03-critic.md'), 'utf8')
    const repair = await readFile(resolve(REPO_ROOT, 'packages/generator/prompts/2.12.0/04-repair.md'), 'utf8')

    for (const stage of [plan, author, critic, repair]) {
      expect(stage).toContain('2.12.0')
  ...
  it('uses CurriculumPackageSchema 2.5.0 while preserving V24, V23, V22, V21, and V20 legacy schemas', async () => {
    ...
    expect(CurriculumPackageV24Schema.shape.metadata.shape.schemaVersion.safeParse('2.4.0').success).toBe(true)
    expect(CurriculumPackageV24Schema.shape.metadata.shape.schemaVersion.safeParse('2.5.0').success).toBe(false)
    ...
    expect(bundle).toContain('bundleVersion: "2.12.0-prod"')
    expect(bundle).toContain('schemaVersion: "2.5.0"')
    expect(bundle).toContain('engineVersion: "1.7.0"')

...
export const FROZEN_2110_FILES = [
...
export const FROZEN_2131_FILES = [
...
export const SOURCE_FILES = [
  ...
  'packages/generator/prompts/2.13.2/01-plan.md',
  'packages/generator/prompts/2.13.2/02-author.md',
  'packages/generator/prompts/2.13.2/03-critic.md',
...
export async function computeFrozen2110Hashes(repoRoot: string = REPO_ROOT): Promise<Record<string, string>> {
  const hashes: Record<string, string> = {}
  for (const relativePath of FROZEN_2110_FILES) {
    const fullPath = resolve(repoRoot, relativePath)
    const content = await readFile(fullPath, 'utf8')
    hashes[relativePath] = createHash('sha256').update(content.replace(/\r\n/g, '\n')).digest('hex')
  }
...
export async function computeFrozen2131Hashes(repoRoot: string = REPO_ROOT): Promise<Record<string, string>> {
  const hashes: Record<string, string> = {}
  for (const relativePath of FROZEN_2131_FILES) {
    const fullPath = resolve(repoRoot, relativePath)
    const content = await readFile(fullPath, 'utf8')
    hashes[relativePath] = createHash('sha256').update(content.replace(/\r\n/g, '\n')).digest('hex')
  }
...
export async function compileProductionBundle(
  ...
  const readPromptStage = async (fileName: string) => {
    const stage = await readFile(resolve(repoRoot, `packages/generator/prompts/2.13.2/${fileName}`), 'utf8')
    return `${stage.trim()}\n`
  }

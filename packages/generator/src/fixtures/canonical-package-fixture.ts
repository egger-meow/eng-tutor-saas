import type { CurriculumPackage, CurriculumPackageV20, CurriculumQuestion } from '../index.js'
...
export function validPackage(): CurriculumPackageV20 {
  ...
  const question = (id: string, itemType: CurriculumQuestion['itemType'] = 'short-response'): CurriculumQuestion => ({
    id,
  ...
  return {
    ...
    learningPlan: {
      ...
      difficultyBand: '國一適中',
      targets: [
        { id: 'reading-inference', domain: 'reading', description: '根據前後因果推論。', evidence: [{ source: 'feedback', detail: '上週閱讀太簡單。' }], successCriteria: '能指出證據並選出合理推論。' },
        { id: 'grammar-do-does', domain: 'grammar', description: '正確使用 do 與 does。', evidence: [{ source: 'grammar', detail: '近期重複答錯。' }], successCriteria: '四題至少答對三題。' },
        { id: 'vocab-experiment', domain: 'vocabulary', description: '在語境使用核心單字。', evidence: [{ source: 'curriculum', detail: '銜接學校進度。' }], successCriteria: '能理解並造句。' },
      ],

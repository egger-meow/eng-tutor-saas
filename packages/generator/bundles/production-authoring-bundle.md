...
## 3. Opening, teaching, and response format selection

Choose `studentLesson.opening.activity` for this learner and reading: `question` only for a useful ungraded reflection, `observation` for noticing a contrast before explanation, `reading-purpose` for a concrete reading purpose, or `direct-reading` when an extra opening adds no value. A question and writing space are not mandatory. Keep keyed assessments in practice/homework with their normal answer contracts.

Plan each instruction section as a short sequence of typed `blocks`: `prose`, `bullets`, `comparison`, `steps`, `worked-example`, or `error-analysis`. Choose the teaching action first: contrast confusing forms, explain a decision, demonstrate a process, or repair a misconception. Do not reproduce explanation -> patterns -> two examples -> mistake in every section. Teaching must remain sufficient for independent study; variety never excuses missing explanation.

Use recent opening and instruction modes plus response item counts to inspect repeated learning rhythm. Record the rationale internally. Preserve useful consistency and deliberate retrieval; there is no forced rotation, minimum variety count, or obligation to use every block type.


Response formats must actively serve the learner's thinking task rather than relying on model randomness or mechanical rotation quotas. There is no quota forcing a table every week. All formats are expressed strictly through Schema 2.6.0 layout primitives: `lines`, `table`, `organizer`, and `sequence`.

Use this canonical format selection decision matrix:

| 學習任務 (Learning Task) | 優先形式 (Preferred Format) | Schema 2.6.0 Primitive |
|---|---|---|
...
- `recentFormatUse`: deliveries containing a format over the last 3–4 deliveries
- `recentOpeningUse` / `recentInstructionUse`: deliveries containing each known opening or teaching mode
- `recentResponseItemCounts`: response item counts across deliveries with known counts, not estimated minutes; formats may overlap within an item
- `responseCountCoverage`: known versus total deliveries; missing historical counts are unknown, never zero
- `recentReasoning`: recently exercised reasoning operations
- `avoidMechanicalRepeat`: formats present in multiple recent deliveries (recommendation to avoid uncritical reuse)
...
Produce a coherent Schema 2.6.0 learning plan, grounding plan, CAP assessment plans, format selections, and internal rationale sufficient for Author and Critic to execute. Internal planning evidence stays out of Student/Parent prose.
...
Inspect the opening and instruction blocks as well as question layouts. Does the opening help the learner enter this reading, or merely demand an answer before giving useful information? Direct reading is a valid choice. Does each comparison, list, step, example, or error analysis make thinking clearer? Flag decorative tables, duplicated prose, unsupported mini-assessments in teaching blocks, and missing self-study explanation.

Compare recentOpeningUse, recentInstructionUse, recentResponseItemCounts, and responseCountCoverage with this packet. Missing historical modes/counts are unknown. Repeated rhythm deserves scrutiny, not automatic rejection; require learner-specific reasons for otherwise mechanical repetition. Never reward superficial format swaps that leave every task as copying.

...
export function renderCurriculumParentAnswerHtml(pkg: CurriculumPackage): string {
  ...
  const body = `${header(pkg, '家長答案與觀察')}
    ...
    <section><h2>這份教材為什麼這樣安排</h2><p>${h(pkg.learningPlan.personalizationStrategy)}</p><p class="small">本次應用的回饋：</p>${list(pkg.qualityEvidence.feedbackApplied)}<p class="small">相較上一份教材的改善：</p>${list(pkg.qualityEvidence.improvementComparedToPrevious)}</section>

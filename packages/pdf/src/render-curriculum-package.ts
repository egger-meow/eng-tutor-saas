...
export function renderCurriculumParentAnswerHtml(pkg: CurriculumPackage): string {
  const body = `${header(pkg, '家長答案')}
    <section class="callout"><div class="kicker">家長只需要做一件事</div><p>孩子完成學生教材後，再把這份答案交給他自行核對。您不需要講課，也不需要逐題追問。</p><p class="small">本週重點：${h(pkg.parentSummary.focusZh)}</p></section>
    <section><h2>答案</h2>${pkg.answers.map((answer) => `<article class="answer"><div class="qid">${h(answer.questionId)}</div><p><strong>答案：</strong>${h(answer.answer)}</p>${answer.acceptedAnswers.length ? `<p><strong>也可接受：</strong>${h(answer.acceptedAnswers.join('；'))}</p>` : ''}<p class="small"><strong>簡短理由：</strong>${h(answer.explanationZh)}</p>${answer.likelyMisconceptionZh ? `<p class="small"><strong>常見誤區：</strong>${h(answer.likelyMisconceptionZh)}</p>` : ''}</article>`).join('')}</section>
    <section class="callout"><div class="kicker">遇到不懂時</div><p>請孩子先找出錯在哪裡；仍不懂時，再拍下題目請 AI 解釋原因並另出一題。詳細方法請看網站的學習指南。</p></section>`

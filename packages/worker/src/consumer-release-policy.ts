import { desiredAuthoringContract, LEGACY_AUTHORING_CONTRACT, PREVIOUS_AUTHORING_CONTRACT, claimAuthoringContract } from './authoring-claim-contract.js'
...
export function validateConsumerRelease(context: Record<string, unknown>, metadata: Record<string, unknown>): string {
  ...
  const supported = [desiredAuthoringContract, PREVIOUS_AUTHORING_CONTRACT, LEGACY_AUTHORING_CONTRACT].find(candidate => candidate.releaseId === target)

...
const promptRoot = new URL('../../generator/prompts/2.4.0/', import.meta.url)
...
export async function buildCurriculumPromptBundle(context: GenerationContext): Promise<string> {
  ...
  return [
    '# 紙屬英文 Curriculum Package 2.2.0 · Prompt 2.4.0 · Production Authoring bundle',
    '',
    '這是一個 production generation context。只產出符合 `CurriculumPackageSchema` (2.2.0) 的 JSON；不要輸出 Markdown、PDF、解釋文字或另一位孩子的資料。',
    ...
    '- Privacy: Never inject real personal identifiers (legal full names, school names, addresses). Use only the child nickname from context.',

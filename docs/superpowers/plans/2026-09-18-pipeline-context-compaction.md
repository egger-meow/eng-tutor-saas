...
**Implementation update (2026-09-18):** 使用者已要求 Implement。已交付第一批的確定性 JSON 去重、逐次還原驗證、adapter 整合及前後量測；詳見 [實作與驗證紀錄](../../evaluations/2026-09-18-context-presentation.md)。後續完成並依使用者指示接入 [stage-aware bundle runtime view](../../evaluations/2026-09-18-stage-aware-bundle-candidate.md)：相對既有 runtime presentation，正常 Author 縮減 18.61%，repair 縮減 12.71%，保留完整 schema、共同規則、Author 與 Critic；原始 bundle、claim hash 與版本不變，每次 attempt 記錄 presentation hashes 與省略 stage。下方「本次只規劃」描述原始規劃回合。Task 4 的配對模型品質實驗仍未執行，不以決定性保留測試宣稱教學效果等價。

...
- `subscriptions`: one entitlement per child, including provider/customer IDs, plan, TWD catalog price, Founder lifecycle (`none`/`eligible`/`redeemed`/`expired`/`forfeited`), 14-day beta trial window, redemption/forfeiture audit timestamps, and billing period state.
- `private_generation.founder_checkout_claims`: server-only 30-minute checkout hold binding between one temporarily consumed Founder seat and the exact discounted Paddle transaction. Active claims (`reservation_expires_at > now()`) are counted toward the 30 Founder limit; a claim transitions to completed upon verified subscription creation or releases after the transaction is neutralized.

...
3. **Terminal Submission Immutability**: Once a generation job has generated ANY `curriculum_submission`, its target identity and attempt sequence are strictly immutable. If an unmaterialized job has terminal submission history (such as `quality_rejected` or `technical_failed`), that job is marked `canceled` (preserving its job and submission audit history), and its `idempotency_key` is suffixed to free the slot. A fresh generation job with a brand new job ID is created with tomorrow's anchor, `status = 'pending'`, and clean `attempt_count = 0`, preventing immutable `(job_id, authoring_attempt)` submission primary key collisions on retry;

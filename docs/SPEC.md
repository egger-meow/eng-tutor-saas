...
The deployed authoring contract is authoritative for new claims; the repository manifest describes the desired release. The verified active release as of 2026-09-11 is rel_1.9.0 (Engine 1.9.0, Schema 2.6.0, Prompt 2.14.0, Worker 1.8.0, Renderer 1.6.0). Supported in-flight predecessor claims retain rel_1.8.2 (Engine 1.8.2, Schema 2.5.0, Prompt 2.13.2, Worker 1.7.2, Renderer 1.5.0). Resolve fresh production contract state for subsequent operations; the activation evidence is recorded in docs/evaluations/2026-09-11-curriculum-diversity-activation.md. Deploy and verify compatible consumers before activation, and preserve each existing claim and submitted artifact under its original contract. The submission bridge also accepts Schema 2.4.0 for in-flight compatible packets; it preserves server-owned claim fingerprints and immutable attempt contents. Prompt 2.14.0 is the consolidated active baseline: production model context reads the current compact plan/author/critic/repair suite directly instead of concatenating historical overlays. Prompt 2.4.0 through 2.11.1 remain frozen historical inputs for provenance and legacy interpretation. Future permanent prompt improvements should update or replace concise sections in the active consolidated baseline rather than resume an indefinitely growing overlay chain.

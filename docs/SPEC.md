...
The deployed authoring contract is authoritative for new claims; the repository manifest describes the desired release. Release rel_1.9.1 keeps Engine 1.9.0, Schema 2.6.0, Worker 1.8.0, and Quality Profile 1.2.0 while advancing Prompt to 2.14.1 and Renderer to 1.6.1. The prior rel_1.9.0 contract (Prompt 2.14.0, Renderer 1.6.0) and supported rel_1.8.2 claims (Engine 1.8.2, Schema 2.5.0, Prompt 2.13.2, Worker 1.7.2, Renderer 1.5.0) retain their immutable bundles and metadata. Resolve fresh production contract state for subsequent operations; activation evidence is recorded in docs/evaluations/2026-09-18-prompt-patch-release.md. Deploy and verify compatible consumers before activation, and preserve each existing claim and submitted artifact under its original contract. The submission bridge also accepts Schema 2.4.0 for in-flight compatible packets; it preserves server-owned claim fingerprints and immutable attempt contents. Prompt 2.14.1 is the consolidated active baseline: production model context reads the compact plan/author/critic/repair suite directly instead of concatenating historical overlays. Prompt 2.14.0 and earlier suites remain frozen historical inputs for provenance and legacy interpretation. Future permanent prompt improvements must create a new release identity rather than mutate an activated prompt directory or resume an indefinitely growing overlay chain.
...
20. Author/Critic and pre-submit validation target the inclusive 85%-115% band. The normal Finisher independently normalizes the canonical package and treats `BUDGET_UNDERFILLED` / `BUDGET_OVERFILLED` as publication-blocking findings. This workload comparison is an explicit product-capacity invariant, not warning-only style telemetry.

...
AUTHORITATIVE CLAIM — NO CATALOG PREFLIGHT

Do not query `pg_catalog`, `information_schema`, `to_regprocedure`, or any other catalog/introspection surface. Production availability must not depend on optional health introspection. The claim RPC itself is the authoritative production boundary and the first database mutation in the run.

Then execute exactly once:

select private_generation.chatgpt_claim_generation_batch('chatgpt-work-daily') as result;

If that exact call is missing or unavailable, claim nothing else and classify the actual connector/database error as PIPELINE_BRIDGE_MISSING or PRECHECK_BLOCKED. Do not attempt a replacement catalog query and do not claim a second time.

The returned JSON is authoritative, including its `bridgeVersion`. It already enforces entitlement, feedback eligibility, normal capacity, mandatory deadline override, ordering, idempotency, and a cloud-authoring lease. Every claimed context includes an `inputFingerprint` generated by Supabase from the exact server-owned context snapshot. Never calculate, replace, normalize, or guess this value. Never add a job manually, never run a second claim in the same task, and never fill spare capacity with a job still waiting for feedback.

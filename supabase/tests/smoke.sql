...
begin
  ...
  if bridge_context ->> 'targetReleaseId' <> 'rel_1.7.0' then
    raise exception 'claim context missing server-owned targetReleaseId rel_1.7.0: %', bridge_context;
  end if;
  ...
  if bridge_context ->> 'targetReleaseId' <> 'rel_1.7.0' then
    raise exception 'retry claim context missing server-owned targetReleaseId rel_1.7.0: %', bridge_context;
  end if;
  ...
  begin
    ...
    if mismatch_context is null or mismatch_context ->> 'targetReleaseId' <> 'rel_1.7.0' then
    ...
    if not public.worker_finish_curriculum_submission(
      ...
      'Release mismatch: submission target release rel_1.4.0 does not match Finisher CURRENT_RELEASE_ID rel_1.7.0'
    ) then
    ...
    -- 7. Verify attempt 2 claim context has targetReleaseId=rel_1.7.0 and NO retryContext (fresh authoring, not quality repair)
    if mismatch_context ->> 'targetReleaseId' <> 'rel_1.7.0'

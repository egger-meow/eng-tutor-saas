...
-- Beta trial and other unredeemed subscriptions reset to founding_status = 'none' and null reservation.
update public.subscriptions
set founding_status = 'none',
    founding_reserved_until = null
where founding_status in ('eligible', 'expired');
...
alter table public.subscriptions
  ...
  add constraint subscriptions_founding_lifecycle_check check (
    (founding_status not in ('redeemed', 'forfeited') or founding_redeemed_at is not null)
...
create index if not exists subscriptions_active_founding_reservation_idx
  on public.subscriptions (founding_reserved_until)
  where founding_status = 'eligible';

comment on column public.subscriptions.founding_reserved_until is 'Legacy reservation field retained for constraint compatibility.';
...
-- Beta trial creation trigger:
-- Normal admitted children are all equal during the free Week 1 stage.
-- No Founder reservation is allocated at child creation time.
-- An unpaid beta trial receives a deterministic 14-day service-trial window.
...
begin
  ...
  -- Forfeited status cannot reacquire Founder pricing.
  if child_subscription.founding_status = 'forfeited' then
    return query select false, 'forfeited'::text, null::uuid, null::text;
    return;
  end if;

  -- Reuse existing unexpired live claim for this child.
  if live_claim.id is not null and live_claim.reservation_expires_at > now() then
    return query select true, coalesce(child_subscription.founding_status, 'none'), live_claim.id, live_claim.paddle_transaction_id;
    return;
  end if;

  -- Check if Founder seats are available to mint a new 30-minute hold.
  if not exists (select 1 from public.children where id = p_child_id and is_internal_test) then
    ...
    if founding_count < settings.founding_limit then
      ...
      return query select true, coalesce(child_subscription.founding_status, 'none'), live_claim.id, null::text;
      return;
    end if;
  end if;
...
-- process_paddle_subscription_event_v2_base
...
begin
  ...
  -- First week material initialization: only if child has no materials and no generation jobs.
...
begin
  ...
  if p_plan_code = 'standard_monthly'
    ...
    and p_discount_id = p_expected_founding_discount_id
    and coalesce(existing_subscription.founding_status, 'none') not in ('redeemed')
  then
    if founder_claim.id is null or founder_claim.status <> 'bound'
      or p_event_type <> 'subscription.created'
      or founder_claim.paddle_transaction_id is distinct from p_originating_transaction_id
      or founder_claim.reservation_expires_at < p_occurred_at then
      raise exception 'Founder redemption requires the exact claimed Paddle transaction';
    end if;
    if not p_discount_ends_at_present or p_discount_ends_at is not null then
      raise exception 'Founder discount must explicitly be forever-recurring';
    end if;
  elsif coalesce(existing_subscription.founding_status, 'none') not in ('redeemed')
    and p_discount_id = p_expected_founding_discount_id
    and p_status in ('trialing', 'active')
  then
...
-- Active service capacity is occupied only by active Paddle subscriptions and valid unexpired beta trials.
-- Expired beta trials release capacity back to remaining.
...
-- private_generation.claim_due_generation_jobs:
-- Unpaid beta trial is strictly Week 1 only (source_material_id is null and no prior material).
-- Week 2+ jobs require a real Paddle subscription activation or internal-test status.

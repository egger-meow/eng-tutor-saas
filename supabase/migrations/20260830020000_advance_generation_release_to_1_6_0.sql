-- Migration: Advance generation release to rel_1.6.0 while preserving Schema 2.4.0 and bridge semantics

-- 1. Update chatgpt_claim_generation_batch to emit targetReleaseId rel_1.6.0
...
-- 2. Keep Schema 2.4.0 submission semantics and stamp rel_1.6.0 from the server-owned claim snapshot
...
begin
  ...
  target_rel_id := coalesce(claim_snapshot.generation_context->>'targetReleaseId', 'rel_1.6.0');

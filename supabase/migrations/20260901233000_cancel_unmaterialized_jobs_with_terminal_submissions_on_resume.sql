-- Migration: Cancel unmaterialized jobs with terminal submissions on entitlement resume
-- Architectural Invariant: Once a generation job has generated ANY curriculum_submission,
-- its target identity and attempt sequence are strictly immutable.
-- When entitlement transitions to active/trialing:
-- 1. In-flight claims (lease valid) or submissions (pending/processing) are untouched.
-- 2. Unmaterialized jobs with terminal submission history (quality_rejected, technical_failed)
--    are marked canceled (preserving job and submission history for audit) and their idempotency_key
--    is suffixed so the anchor slot is freed.
-- 3. Only clean jobs with zero submission history are re-anchored in place.
-- 4. If no clean job exists, a fresh generation job with tomorrow's anchor and fresh attempt counter (0)
--    is created, avoiding immutable attempt/submission PK collisions.
  ...
  then
    ...
    if v_child_tz is not null then
      ...
      if not exists (
        ...
        -- 2. Terminal submission history protection:
        -- Once a generation job has generated ANY curriculum_submission, its target identity is immutable.
        -- If an unmaterialized job has terminal submission history (quality_rejected, technical_failed, etc.),
        -- preserve its job record and submissions for audit, but cancel the unmaterialized job.
        -- Update its idempotency_key to free the anchor slot for the successor job.
        ...
        -- ONLY unmaterialized jobs with ZERO submission history can be re-anchored in place.
        ...
        if v_candidate_job.id is not null then
          ...
          -- jobs had terminal submissions and were safely canceled to protect immutable attempts).

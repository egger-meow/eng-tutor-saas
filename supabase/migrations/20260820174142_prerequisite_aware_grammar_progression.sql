-- Add a stable canonical order to the derived grammar map and fill the one missing Appendix 6 family.
alter table public.canonical_grammar_units
  add column if not exists progression_order integer;

insert into public.canonical_grammar_units (unit_id, grade_stage, prerequisites, progression_order)
values
  ('g7-be-verbs-pronouns', 'grade_7', ARRAY[]::text[], 1),
  ('g7-present-simple-verbs', 'grade_7', ARRAY['g7-be-verbs-pronouns']::text[], 2),
  ('g7-do-does-questions', 'grade_7', ARRAY['g7-present-simple-verbs']::text[], 3),
  ('g7-modals-ability-permission', 'grade_7', ARRAY['g7-present-simple-verbs']::text[], 4),
  ('g7-present-continuous', 'grade_7', ARRAY['g7-be-verbs-pronouns']::text[], 5),
  ('g7-imperatives', 'grade_7', ARRAY[]::text[], 6),
  ('g7-there-is-are', 'grade_7', ARRAY['g7-be-verbs-pronouns']::text[], 7),
  ('g7-time-place-prepositions', 'grade_7', ARRAY[]::text[], 8),
  ('g7-wh-questions', 'grade_7', ARRAY['g7-do-does-questions']::text[], 9),
  ('g8-past-simple-verbs', 'grade_8', ARRAY['g7-present-simple-verbs']::text[], 10),
  ('g8-past-continuous', 'grade_8', ARRAY['g8-past-simple-verbs','g7-present-continuous']::text[], 11),
  ('g8-future-tense', 'grade_8', ARRAY['g7-be-verbs-pronouns']::text[], 12),
  ('g8-modals-obligation-advice', 'grade_8', ARRAY['g7-present-simple-verbs']::text[], 13),
  ('g8-adverbial-clauses-time-reason', 'grade_8', ARRAY['g8-past-simple-verbs']::text[], 14),
  ('g8-gerunds-infinitives', 'grade_8', ARRAY['g7-present-simple-verbs']::text[], 15),
  ('g8-comparatives-superlatives', 'grade_8', ARRAY['g7-be-verbs-pronouns']::text[], 16),
  ('g8-sense-causative-verbs', 'grade_8', ARRAY['g8-past-simple-verbs']::text[], 17),
  ('g9-present-perfect-experience', 'grade_9', ARRAY['g8-past-simple-verbs']::text[], 18),
  ('g9-present-perfect-duration', 'grade_9', ARRAY['g9-present-perfect-experience']::text[], 19),
  ('g9-passive-voice', 'grade_9', ARRAY['g8-past-simple-verbs','g9-present-perfect-experience']::text[], 20),
  ('g9-noun-clauses-that', 'grade_9', ARRAY['g8-adverbial-clauses-time-reason']::text[], 21),
  ('g9-indirect-questions', 'grade_9', ARRAY['g7-wh-questions','g9-noun-clauses-that']::text[], 22),
  ('g9-relative-clauses-subject', 'grade_9', ARRAY['g8-gerunds-infinitives']::text[], 23),
  ('g9-relative-clauses-object', 'grade_9', ARRAY['g9-relative-clauses-subject']::text[], 24),
  ('g9-conditionals', 'grade_9', ARRAY['g8-future-tense','g8-adverbial-clauses-time-reason']::text[], 25)
...
alter table public.canonical_grammar_units
  alter column progression_order set not null;

create unique index if not exists canonical_grammar_units_progression_order_key
  on public.canonical_grammar_units (progression_order);
...
begin
  if not exists (
    select 1
    from pg_constraint
    where conname = 'canonical_grammar_units_progression_order_positive'
      and conrelid = 'public.canonical_grammar_units'::regclass
  ) then
    alter table public.canonical_grammar_units
      add constraint canonical_grammar_units_progression_order_positive
...
begin
  ...
  -- 2. Recommended Grammar Gap Fill: unlock only mastered prerequisites, then follow canonical progression.
  select coalesce(jsonb_agg(sub.unit_id order by sub.is_new desc, sub.progression_order), '[]'::jsonb)
  into v_rec_grammar
  from (
    select
      cgu.unit_id,
      (cgp.grammar_id is null) as is_new,
      cgu.progression_order
    ...
    where (cgp.grammar_id is null or cgp.status in ('learning', 'reviewing'))
      ...
      and not exists (
        select 1
        from unnest(cgu.prerequisites) as prerequisite(unit_id)
        where not exists (
          select 1
          from public.child_grammar_progress as prerequisite_progress
          where prerequisite_progress.child_id = claimed_job.child_id
            and prerequisite_progress.grammar_id = prerequisite.unit_id
            and prerequisite_progress.status = 'mastered'
        )
      )
    order by is_new desc, cgu.progression_order
    limit 2
  ) as sub;
    ...
    'capCoverageCapsule', jsonb_build_object(
      ...
      'coverage', jsonb_build_object(
        ...
        'grammar', jsonb_build_object(
          ...
          'exposurePct', round((v_grammar_exp_count::numeric / 25.0) * 100.0, 1),
          'masteryPct', round((v_grammar_mastered_count::numeric / 25.0) * 100.0, 1)
        ),

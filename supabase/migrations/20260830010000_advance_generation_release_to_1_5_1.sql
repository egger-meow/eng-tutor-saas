-- Migration: Advance generation release to rel_1.5.1 and accept Schema 2.4.0 in ChatGPT submission bridge

-- 1. Update chatgpt_claim_generation_batch to emit targetReleaseId rel_1.5.1
...
-- 2. Update chatgpt_submit_curriculum_package to accept Schema 2.4.0 and default to rel_1.5.1
...
begin
  ...
  if jsonb_typeof(canonical_source) <> 'object'
    ...
    raise exception 'canonical_source must be a Curriculum Package 2.4.0 object';
  end if;
  ...
  target_rel_id := coalesce(claim_snapshot.generation_context->>'targetReleaseId', 'rel_1.5.1');

-- Migration: Harden subscription pause clock and active generation guards
-- 1. Restrict pre-claim re-anchoring strictly to entitlement transitions (beta -> active, paused/canceled -> active, none -> active).
--    Continuous active subscribers (active -> active, trialing -> active) retain rolling weekly cadence; overdue jobs remain mandatory.
-- 2. Guard active curriculum submissions (pending/processing) across ALL candidate jobs (both pending and expired claimed).
-- 3. Resolve latest material authority via child_weekly_learning_snapshots.sequence_number canonical chain.
...
begin
  ...
  then
    ...
    if v_child_tz is not null then
      ...
      -- ONLY pending jobs, or claimed jobs whose lease expired.
      -- Active worker claims (lease valid) and active submissions (pending or processing)
      -- across ALL candidate job statuses are strictly protected and never touched.
      ...
      if v_candidate_job.id is not null then
        ...
        -- (safeguarding any job that might hold an active in-flight submission)
        ...
        where other_job.child_id = p_child_id
          ...
          and not exists (
            select 1 from private_generation.curriculum_submissions as s
            where s.job_id = other_job.id and s.status in ('pending', 'processing')
          );
        ...
        -- Canonical chain authority: prioritize highest sequence_number from child_weekly_learning_snapshots.
        ...
        -- Fallback to materials table ordering only if no snapshot exists
